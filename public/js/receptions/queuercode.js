...
$(document).ready(function(){
	...
	$(document).on('click', '#qcodeModal', function() {
		$('#qcodeInput').focus();
	});

	$(document).on('keypress', '.receptionsbarcode', function(e){
		if(e.keyCode == '13') {
			e.preventDefault();
			$('#qrcodeModal').modal('toggle');

			let code = $(this).val();
			let status = $('.loadPatients').attr('charoff');
			let stringLength = status.length; // this will be 16
			status = status.charAt(stringLength - 1); 
		    openLoader();

			console.log('status: ',status);
			$.ajax({
				url: baseUrl+"/receptionsbarcode",
				type: "POST",
				data: {
					barcode:code,
					status:status
				...
				success: function(data) {
		            ...
		            if(data != 0 && data != 1  && data != 2)
		            {
						...
						tdBtnClass, eturnConfirm = '', returnConfirm1 = '', cancelClass = '', patientID = '';
						// count = 1;
						let patients = data;
						...
						trCount = 0;
						let boolean;
						$('.loadPatients tr').each(function(){
							boolean = $(this).hasClass('nopatient');
							status != 'P' && status != 'H' && status != 'C' && status != 'S' && status != 'F' ? boolean ? $(this).remove() : '' : '';
							trCount++;
						});

						console.log('boolean: ', $(this).hasClass('nopatient'));
						...
						if(numPatient > 0)
						{
							...
							patients.forEach( (patient) => {
								boolean ? trCount : trCount++;
								status = 'cancel';
								tdContentInfo = '';
								tdContentInfo2 = '';
								$('.nopatient').fadeOut(300,function() {
									$(this).remove();
								});
								...
								if(patient.userClinic != 31)
								{
									if(patient.status == 'S' || patient.status == 'P' || patient.status == 'F' || patient.status == 'H')
									{
										...
										{
											td3Div = '<div class="online"></div> <span class="text-default">Dr. '+patient.doctorsname.toUpperCase()+'</span>';
										}
										else
										{
											td3Div = '<div class="offline"></div> <span class="text-default">Dr. '+patient.doctorsname.toUpperCase()+'</span>';
										}
									}
									else
									{
						...
						status != 'P' && status != 'H' && status != 'C' && status != 'S' && status != 'F' ? $('.loadPatients').append(tr_tag) : '';
			            toastr.success('Patient Successfully Assigned.');
						countConsultation(patientID);
						loadCharging(patientID,charge = '');
		            }
		            else if(data == '')
		            {
		            	toastr.success('Patient Successfully Assigned.');
		            ...
		            {
		            	toastr.error('Patient is already on the queue list.');
		            }
		            else if (data == 2) 
		            {
		            	toastr.error('The patient should pass through the Triage area for clinic assignment.');
		            }
		            else
		            {
