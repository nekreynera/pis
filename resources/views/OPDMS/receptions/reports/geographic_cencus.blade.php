        ...
        <div class="container">
        	<form class="form-inline" method="GET">
        		...
        		<div class="form-group">
        			...
        			<input type="date" name="from" class="form-control" value="{{ (isset($request->from) || $request->from)?Carbon::parse($request->from)->format('Y-m-d'):'' }}">
        		</div>
        		<div class="form-group">
        			<label>TO <small><i>(date end)</i></small></label>
        			<input type="date" name="to" class="form-control" value="{{ (isset($request->to) || $request->to)?Carbon::parse($request->to)->format('Y-m-d'):'' }}">
        		</div>
        		...
        		<table class="table table-bordered" id="geographic-table">
        			...
        			<tbody>
        				...
        				<tr>
                            ...
                            ?>
        					<td class="text-center">{{ ($male > 0)?($male-$revisitmale):'' }}</td>
        					<td class="text-center">{{ ($revisitmale > 0)?$revisitmale:'' }}</td>

        					<td class="text-center">{{ ($female > 0)?($female-$revisitfemale):'' }}</td>
                            <td class="text-center">{{ ($revisitfemale > 0)?$revisitfemale:'' }}</td>
                            
                            <td class="text-center">{{ ($oldmale > 0)?($oldmale - $revisitoldmale):'' }}</td>
        					<td class="text-center">{{ ($revisitoldmale > 0)?$revisitoldmale:'' }}</td>

                            <td class="text-center">{{ ($oldfemale > 0)?($oldfemale - $revisitoldfemale):'' }}</td>
        					<td class="text-center">{{ ($revisitoldfemale > 0)?$revisitoldfemale:'' }}</td>

                            <td class="text-center">{{ ($oldermale > 0)?($oldermale - $revisitoldermale):'' }}</td>
        					<td class="text-center">{{ ($revisitoldermale > 0)?$revisitoldermale:'' }}</td>

                            <td class="text-center">{{ ($olderfemale > 0)?($olderfemale - $revisitolderfemale):'' }}</td>
        					<td class="text-center">{{ ($revisitolderfemale > 0)?$revisitolderfemale:'' }}</td>

        					<td class="text-center">{{ ($totals > 0)?$totals:'' }}</td>
        				</tr>
        				@endforeach
        			</tbody>
